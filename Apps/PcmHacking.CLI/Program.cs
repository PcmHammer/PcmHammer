...
if (result == 0)
{
    ...
    Response<bool> response = await controllerManager.BeginAction();
    if (actionArgs.SelectedAction == ControllerActions.Read && response.Status == ResponseStatus.Unverified)
    {
        outputPath = GetBadReadPath(outputPath);
        logger.AddUserMessage("##############################################################################");
        logger.AddUserMessage("WARNING: Verification timed out. File could not be validated and may be corrupt.");
        logger.AddUserMessage("Saved to " + outputPath + " for debugging only. Do not use this file without validation.");
        logger.AddUserMessage("##############################################################################");
    }
...
static string GetBadReadPath(string path)
{
